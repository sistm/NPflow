...
Flimited<-function(n_small_clst,partition_ref,partition_est){
  ...
  find_index_obs_coclust[index_obs_in_small_class_ref] <- FALSE
  index_obs_in_small_class_ref_2 <- which(find_index_obs_coclust==TRUE)
  
  partition_ref_restricted <- c(partition_ref[sort(c(index_obs_in_small_class_ref,
                                                     index_obs_in_small_class_ref_2))])
