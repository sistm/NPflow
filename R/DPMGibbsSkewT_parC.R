...
#'  fparC <- function(Nit){
#'      set.seed(123)
#'      MCMCsample_st <- DPMGibbsSkewT_parC(z, hyperG0, a, b, N=Nit, 
#'      doPlot=FALSE, nbclust_init=200, diagVar=FALSE, Ncpus=4, verbose=FALSE)
#'  }
#'  
#'  fseq <- function(Nit){
#'      set.seed(123)
#'      MCMCsample_st <- DPMGibbsSkewT(z, hyperG0, a, b, N=Nit, 
#'      doPlot=FALSE, nbclust_init=200, diagVar=FALSE, verbose=FALSE)
#'  }
#'  
#'  fpar <- function(Nit){
#'      set.seed(123)
#'      MCMCsample_st <- DPMGibbsSkewT_parallel(Ncpus=4, type_connec="SOCK", z, hyperG0, a, b, N=Nit, 
#'      doPlot=FALSE, nbclust_init=200, diagVar=FALSE, verbose=FALSE)
#'  }
#'  
...
#'
DPMGibbsSkewT_parC <- function (z, hyperG0, a, b, N, doPlot=TRUE, 
    ...
    logposterior_list[[i]] <- logposterior_DPMST_parC(z, xi=U_xi, psi=U_psi, Sigma=U_Sigma, df=U_df, B=U_B,
                                                 hyper=hyperG0, c=c, m=m, alpha=alpha[i], n=n, a=a, b=b, diagVar=diagVar, Ncpus=Ncpus)
    ...
    if(N>1){
        for(i in 2:N){
            ...
            slice <- sliceSampler_skewT_parC(c=c, m=m, alpha=alpha[i], 
                                        z=z, hyperG0=hyperG0, 
                                        U_xi=U_xi, U_psi=U_psi, 
                                        U_Sigma=U_Sigma, U_df=U_df,
                                        scale=sc, diagVar=diagVar, Ncpus=Ncpus)
            m <- slice[["m"]]
            c <- slice[["c"]]        
