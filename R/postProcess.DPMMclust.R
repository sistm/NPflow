...
postProcess.DPMMclust <- function(x, burnin=0, thin=1, gs=NULL, lossFn="F-measure", K=10,...){
  ...
  if(x$clust_distrib=="skewT"){
  ...
  }else if (x$clust_distrib=="Normal"){
    ...
    for (i in 1:length(param_post$U_mu)){
      parameters[[i]] <- list("mu" = param_post[["U_mu"]][[i]],
  ...
  else {stop("clust_distrib is neither skewT nor Normal\n other distrib not implemented yet")}
...
#     weights <- rep(1/K, K)
    ...
    for(k in 1:K){
      ...
      #       U_xi_U_psi_ <- 0
      #       for (j in 1:N){
      #         U_xi_U_psi_ <- U_xi_U_psi_+t(matrix(r_xi_psi[j,],d,d))%*%rSinv_list[[j]]    
      #       }
      #       U_xi_U_psi <- U_xi_U_psi_%*% solve(rSinv_sum) 
...
MLE_N_mmEM <- function( mu_list, S_list, hyperG0, K, maxit=100, tol=1e-1, plot=TRUE){
  ...
  for(i in 1:maxit){
    ...
    loglik[i+1]<-sum(log(apply(mmNiWpdfC(Mu=sapply(mu_list, "["), Sigma=S_list, 
                                         U_Mu0=sapply(U_mu, "["), 
                                         U_Kappa0=sapply(U_kappa, "["), 
                                         U_Nu0=sapply(U_nu, "["), 
                                         U_Sigma0=U_Sigma,Log=FALSE),
                               MARGIN=2, FUN=function(x){sum(x*weights)})))
    
    
    #cat("it ", i, ": loglik = ", loglik[i+1],"\n\n", sep="")
    if(abs(loglik[i+1]-loglik[i])<=tol & max(loglik[1:i])<loglik[i+1]){break}
    if(plot){
      plot(y=loglik[2:(i+1)], x=c(1:i), 
           ylab="Log-likelihood", xlab="Iteration", type="b", col="blue", pch=16)
    }
...
#' 
#' 
#'mle_niw<-MLE_NiW(U_mu, U_Sigma)
#'mle_niw


MLE_NiW <- function(mu_list, S_list){
  ...
  U_mu<-Reduce('+',lapply(1:N, 
                          function(i,m) {t(mu_list[[i]])%*%m[[i]]}, 
                          m=Sinv_list))%*% solve(Sinv_sum) 
  
  mu_m <- lapply(mu_list, function(x){x - as.vector(U_mu)})
  
  
  U_kappa<-(N*d)/sum(mapply(x = mu_m, Sinv = Sinv_list,
                            FUN=function(x,Sinv){
                              t(x)%*%Sinv%*%x  
                            }, SIMPLIFY=TRUE))
  
  const_nu0_uniroot <-(sum(sapply(S_list, function(S){log(det(S))}))
                       + N*log(det(Sinv_sum)))
  
  max_upper<-1e12
  
  U_nu <- try(uniroot(function(nu0){(N*digamma_mv(x=nu0/2, p=d)
                                     - N*d*log(N*nu0/2) 
                                     + const_nu0_uniroot
  )}, lower = d+1, upper=max_upper)$root, TRUE)
  
  
  if(inherits(U_nu, "try-error")){U_nu <- d+1}
  U_Sigma <- N*U_nu*solve(Sinv_sum)
  
  return (list("U_mu"=U_mu,
               "U_kappa"=U_kappa,
               "U_nu"=U_nu,
               "U_Sigma"=U_Sigma))
