sliceSampler_skewT_parC <- function(c, m, alpha, z, hyperG0, U_xi, U_psi, 
                               U_Sigma, U_df, scale, diagVar, Ncpus){
    ...
    if(length(fullCl_ind)>1){
        l <- mmvstpdfC_par(x=z, xi=U_xi_full, psi=U_psi_full, sigma=U_Sigma_full, df=U_df_full, Log = FALSE, ncores=Ncpus)
